    ...
    if (qEl && aEl && !reduce) {
      var script = [
        { t: "So, how would you make a payment retry safe if the network drops mid-request?",
          a: "Make the write idempotent. I attach an idempotency key per payment, so a retried request maps to the same ledger entry instead of double-charging, the same offline-first pattern I built for UROCK's sync." },
      ];
