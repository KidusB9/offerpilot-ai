  ...
  fetch(base + "/data/checkout.json", { cache: "no-store" })
    ...
    .then(function (cfg) {
      ...
      buttons.forEach(function (el) {
        ...
        // Purchases are tied to a signed-in account, so paid plans funnel through
        // the app: download -> sign in -> upgrade in-app (live Stripe checkout).
        el.setAttribute("data-checkout", "ready");
        if (!configured && plan !== "free") {
          el.setAttribute("title", "Download the app, sign in, and upgrade to the " + plan + " plan in-app.");
        }
      });
    })
